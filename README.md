...
This repository deploys and configures the **engine-egpu** LXC on the HLH Proxmox
...
- LXC 131, hostname `hlh-ai-engine-egpu`, IP `192.168.1.11` (gw 192.168.1.1)
...
# also at /srv/ai/models/egpu-switch-model.sh
nvidia-smi -L; nvidia-smi
```

> Note: `hlh-ai-engine` (112) and `hlh-ai-engine-egpu` (131) share `/srv/ai/models`. The OCuLink slot is exclusive to 131 (former LXC 130 vulkan retired).

## Deployment Model

Two bash scripts only (no ansible/opentofu):

1. **Provisioning**: `deploy-hlh-ai-engine-egpu.sh` creates the privileged LXC, wires CUDA passthrough (`/dev/nvidia*` — single GV100), and pushes the configuration script. If host `nvidia` driver not loaded, it blacklists `nouveau`, ensures trixie non-free + CUDA debian13 repo, and installs `R580 Tesla 580.65.06` via `.run --dkms` (6.14 LTS kernel, Volta last) or `550` on `6.5` via DKMS. Last driver for Volta is R580 (580.65.06) with CUDA 12.8/12.9 - trixie stable has 550, 590+ drops Volta; use `580.65.06` Tesla .run on `6.14`.
2. **Configuration**: `configure-hlh-ai-engine-egpu.sh` - when run on host it pushes itself into the LXC via `pct exec`/`ssh` and re-runs with `--bootstrap-inside`; that flag runs the embedded bootstrap (CUDA toolkit + llama.cpp `sm70 FA ON` + `egpu-switch-model.sh` generation). No separate inside file.
...
| Single slot | OCuLink c5:00.0 — exclusive to this engine (former LXC 130 vulkan retired) |

## Repository Layout

```
hlh-ai-engine-egpu/
├── deploy-hlh-ai-engine-egpu.sh    # Provision: LXC creation + CUDA passthrough + bootstrap (bash)
├── configure-hlh-ai-engine-egpu.sh # Configuration: host wrapper + embedded bootstrap --bootstrap-inside (bash only)
├── 00_BACKLOG.md
├── 10_ACTIVE.md
├── 90_DONE.md
├── CHANGELOG.md
...
| `--ctx-size` | `32768` (32K) | Context window (switch via `egpu-switch-model.sh`) |
...
Context size options (via `egpu-switch-model.sh`):
...
`egpu-switch-model.sh` waits up to 90×2s = 180s probing `/health` and aborting on `ActiveState=failed` / `NRestarts` bump.
